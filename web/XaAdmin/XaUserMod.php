...
?>
	...
	<?php
               $XaUser=new XaUser();
               $WsData=$XaUser->ExecuteSync($Conf,$HTTP,'GetXmlModel','XaUser','{"type":"Update","id":"'.$id.'"}');

               $Tpl = new XaTplUpdate();
               // obj,TargetId,ResponseType belong to form action
               $Content = $Tpl->Update($Conf,$HTTP,$WsData,'{"id":"'.$id.'","obj":"XaUser","WithAlert":"yes","ResponseType":"Text"}');
               echo $Content;
        ?>
    	</div> 
